...
# FIX: proxy/cookies are now read from env so they're easy to toggle
def get_proxy_args():
    proxy = os.environ.get('YT_PROXY', 'socks5://127.0.0.1:1080').strip()
...
def download_video(method, url, tmp_dir, fmt, quality):
    ...
    # FIX: res:desc so we always prefer highest resolution; don't force h264
    # (h264 caps at 1080p on YouTube, blocking 1440p/2160p entirely)
    sort_opt = ["--format-sort", "res:desc"] if quality != "audio" else []

    js_github = ["--js-runtimes", "deno", "--remote-components", "ejs:github"]
    js_npm    = ["--js-runtimes", "deno", "--remote-components", "ejs:npm"]

    # FIX: reordered — tv_embedded, android_vr, ios, tv FIRST (bypass SABR),
    # web/web_safari only as last resorts because they get throttled to 360p.
    methods = {
        1: ["yt-dlp"] + proxy + cookies + ["--format", fmt] + sort_opt + common +
           ["--extractor-args", "youtube:player_client=tv_embedded"] + ua_chrome + lang + [url],

        2: ["yt-dlp"] + proxy + cookies + ["--format", fmt] + sort_opt + common +
        ...
        4: ["yt-dlp"] + proxy + cookies + ["--format", fmt] + sort_opt + common +
           ["--extractor-args", "youtube:player_client=tv"] + ua_chrome + [url],

        5: ["yt-dlp"] + proxy + cookies + ["--format", fmt] + sort_opt + common +
           ["--extractor-args", "youtube:player_client=mweb"] + ua_android + [url],

        6: ["yt-dlp"] + proxy + cookies + ["--format", fmt] + sort_opt + common + js_github +
           ["--extractor-args", "youtube:player_client=web,tv_embedded"] + ua_chrome + lang + [url],

        7: ["yt-dlp"] + proxy + cookies + ["--format", fmt] + sort_opt + common + js_npm +
           ["--extractor-args", "youtube:player_client=web_safari"] + ua_safari + lang + [url],
...
def create_readme(folder_path, filename, url, quality, parts_info, has_password, is_split):
    ...
    readme += "\n---\n\n"          # FIX: removed avasam credit line
...
def process_video(url, quality, password, backup_dir, repo_owner, repo_name, branch, url_index):
    ...
    # FIX: track the best result across attempts instead of rejecting outright
    min_height = 0
    ...
    # FIX: if nothing met the target, re-download using the best method we saw
    ...
    for filepath in Path(tmp_dir).iterdir():
        ...
        if size > SPLIT_BYTES:
            ...
            # FIX: broader pattern so .s00/.t00 etc. are caught if the file is huge
            parts = sorted(
...
def download_subtitles(url, folder_path, folder_name, repo_owner, repo_name, branch):
    ...
    def sub_download(mode):
        ...
        # FIX: put SABR-bypassing clients first, same reorder as video
        for method in range(1, 9):
            methods = {
                1: ["yt-dlp"] + proxy + cookies + ["--extractor-args", "youtube:player_client=tv_embedded"] + flags + common + [url],
                2: ["yt-dlp"] + proxy + cookies + ["--extractor-args", "youtube:player_client=android_vr"] + flags + common + [url],
                3: ["yt-dlp"] + proxy + cookies + ["--extractor-args", "youtube:player_client=ios"] + flags + common + [url],
                4: ["yt-dlp"] + proxy + cookies + ["--extractor-args", "youtube:player_client=tv"] + flags + common + [url],
                5: ["yt-dlp"] + proxy + cookies + ["--extractor-args", "youtube:player_client=mweb"] + flags + common + [url],
                6: ["yt-dlp"] + proxy + cookies + ["--extractor-args", "youtube:player_client=web,tv_embedded", "--js-runtimes", "deno", "--remote-components", "ejs:github"] + flags + common + [url],
                7: ["yt-dlp"] + proxy + cookies + ["--extractor-args", "youtube:player_client=web_safari", "--js-runtimes", "deno", "--remote-components", "ejs:npm"] + flags + common + [url],
    ...
    if os.path.exists(readme_path):
        ...
        # FIX: was looking for "## Download Link" (singular) — actual header is plural
